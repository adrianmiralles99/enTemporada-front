import { Receta } from '../app/receta';

export const RECETAS: Receta[] = [
    { id: 1, titulo: 'Tarta de Manzana', autor: 'alturano', img: "tartaManzana" },
    { id: 2, titulo: 'Hummus de Aguacate', autor: 'adrimigo', img: "hummus" },
    { id: 3, titulo: 'Boniatos asados rellenos', autor: 'ByCharles', img: "boniatos" },
    { id: 4, titulo: 'Sopa de remolacha y yogur', autor: 'el_vikingo', img: "sopaRemolacha" },
    { id: 5, titulo: 'Ensalada de calabaza asada', autor: 'Juan Monta Puercos', img: "ensalada" },
    { id: 6, titulo: 'Banana Split', autor: 'Jeni Electrica', img: "banana" },
];